...
to_compare_filter = {"model": "IMAGE", "scenario": "SSP1-19"}
...
save_plots = "../Output/CruncherResults/"
...
real_results = db_all.filter(**to_compare_filter)
db = db_all.filter(**to_compare_filter, keep=False)
vars_to_crunch = [req for req in db.filter(level=1).variables() if req not in leaders]
...
for cruncher_ind in range(len(crunchers_list)):
    ...
    for var_inst in vars_to_crunch:
        ...
        originals = real_results.filter(variable=var_inst).data.set_index("year")[
            "value"
        ]
