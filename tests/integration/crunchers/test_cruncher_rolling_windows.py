...
# tests to write:
#   - derive relationship with lead and follower being the same gas gives a filler which just gives back same pathways you put in
...
class TestDatabaseCruncherRollingWindows(_DataBaseCruncherTester):
    ...
    def test_derive_relationship_error_quantile_out_of_bounds(self, test_db, nwindows):
        tcruncher = self.tclass(test_db)
        error_msg = re.escape(
    ...
    def test_derive_relationship_error_quantile_out_of_bounds(
